...
```
生成 K 个候选 → HybridScorer 评分排序 → 裁判对比高低分 → 提取通用写作规则 → 注入下一轮 prompt
```
...
评分器称为 **HybridScorer**，综合嵌入语义覆盖和实体完整性两个维度。
...
├── evaluator/
│   ├── hybrid_scorer.py # HybridScorer 评分器（嵌入 + NLI）
│   └── ner.py           # 实体抽取（spaCy）
├── generator/
...
2. **运行速度受 API 限制**：每轮需 5 次生成 + 1 次裁判共 6 次 LLM 调用，耗时约 40-60 秒/轮，瓶颈全在等待 API 响应
3. **质量提升幅度有限**：当前实验中迭代有正向趋势但不排除随机性，部分对话甚至出现早停。评分器与生成目标的对齐程度可能是核心瓶颈

## 参考

- [3DrawAgent](https://arxiv.org/abs/2501.09153) — 对比知识提取（CKE）+ 经验库迭代
- HybridScorer — 本项目内置评分器，综合嵌入语义覆盖 + 实体完整性评估
